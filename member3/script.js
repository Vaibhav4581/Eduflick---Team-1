...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Find corresponding mentor record using case-insensitive name match
  const exactName = profile.name;
  const { data: mentorRec } = await db.from('mentors').select('id, name').ilike('name', exactName).single();
  
  window.actualMentorId = mentorRec ? mentorRec.id : null;
  const name = mentorRec?.name || exactName || session.user.email || 'Mentor';

  // Set mentor name in UI
...
async function loadStudents() {
  ...
  const { data: profiles, error: pErr } = await db
    ...
    `)
    .eq('role', 'student')
    .eq('mentor_id', window.actualMentorId);
...
async function loadSubmissions() {
  ...
  const { data, error } = await db
    .from('submissions')
    .select(`
      id, project_title, github_url, portfolio_url, file_url, description, status, submitted_at,
      user_id,
  ...
  if (error) { 
    console.error('loadSubmissions error:', error.message); 
    alert('loadSubmissions error: ' + error.message);
    return; 
  }
...
function renderSubmissions() {
  ...
  filtered.forEach(sub => {
    ...
    const linksHtml = [
      ...
      sub.file_url  && `<a href="${esc(sub.file_url)}" target="_blank" class="sub-link" style="color:var(--primary);"><i class="ti ti-file-text"></i> Show files</a>`
    ].filter(Boolean).join('');
    ...
    tr.innerHTML = `
      ...
      <td>
        ${linksHtml ? `<div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">${linksHtml}</div>` : '<span style="color:var(--text-secondary)">No links</span>'}
      </td>
...
async function submitFeedback() {
  ...
  // Get mentor profile and map to mentors table id
  const { data: profile } = await db.from('profiles').select('id, name').eq('id', session.user.id).single();
  let mentorId = null;
  if (profile?.name) {
    const { data: mentorRec } = await db.from('mentors').select('id').ilike('name', profile.name).single();
    mentorId = mentorRec?.id;
  }
  
  if (!mentorId) {
    showToast('Error: Could not find your mentor record in the database.', 'error');
    return;
  }
